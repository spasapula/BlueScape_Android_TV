...
public class HeDeleteMessageSender extends BaseMessageSender {
	private static final String TAG = HeDeleteMessageSender.class.getSimpleName();

	// server <-- client
	// [client-id, "he", target-id, "delete", {"hidden":true}]
	private JSONArray mWebSocketMessage;

	public HeDeleteMessageSender(BaseWidgetModel baseWidgetModel) {
		try {
			...
			mWebSocketMessage.put(baseWidgetModel.getID()); // HISTORY_EVENT_FROM_CLIENT_MESSAGE_FORMAT
															// . TARGET_ID
			mWebSocketMessage.put("delete"); // HISTORY_EVENT_FROM_CLIENT_MESSAGE_FORMAT
												// . EVENT_TYPE
