...
/*
 ...
 https://mufumbo.wordpress.com/2011/01/29/upload-using-multipart-post-using-httpclient-in-android/
 https://mufumbo.wordpress.com/2011/01/
 */
...
public class SimpleMultipartEntity implements HttpEntity {
	...
	public void consumeContent() throws IOException, UnsupportedOperationException {
		if (isStreaming()) { throw new UnsupportedOperationException("Streaming entity does not implement #consumeContent()"); }
	}
	...
	public long getContentLength() {
		...
		for (FilePart filePart : fileParts) {
			...
			if (len < 0) { return -1; // Should normally not happen
			}
			contentLen += len;
		}
	...
	private void addPart(String key, String value, String contentType) {
		try {
			out.write(boundaryLine);
	...
	private void addPartWithCharset(String key, String value, String charset) {
	...
	private byte[] createContentDisposition(String key, String fileName) {
		return (AsyncHttpClient.HEADER_CONTENT_DISPOSITION + ": form-data; name=\"" + key + "\"" + "; filename=\"" + fileName + "\"" + STR_CR_LF)
			.getBytes();
	}
